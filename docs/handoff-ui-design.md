...
- `/plan-design-review`로 컴포넌트 정보 구조, 상태 소유권, 상호작용, 반응형과 접근성 계약을 검토한다.
- 시각 시스템은 이미 승인되었으므로 변경하지 않는다.
- 새로운 시각 언어가 필요하다는 사용자 승인이 없는 한 `/design-consultation`을 다시 사용하지 않는다.
- `/design-shotgun`은 사용자 동의 없이는 사용하지 않는다.
- 같은 단계에서 구현 워크플로를 병행하지 않는다.
